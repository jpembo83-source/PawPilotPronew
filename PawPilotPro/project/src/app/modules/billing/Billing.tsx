...
import { PackagesDashboard } from '../packages/pages/PackagesDashboard';
import { PolicyPortal } from '../policies';
...
const TAB_PERMISSIONS: Record<BillingTab, { module: string; action: 'view' | 'create' | 'update' | 'delete' | 'export' | 'approve' }[]> = {
  ...
  staff: [{ module: 'billing', action: 'view' }],
  settings: [{ module: 'billing', action: 'update' }],
};
...
function SettingsPlaceholder() {
  return (
    <div className="text-center py-12">
      ...
      <h3 className="font-medium text-slate-900">Billing Settings</h3>
      <p className="text-sm text-slate-500 mt-2">
        Configure billing preferences (see Settings → Billing & Finance for full configuration)
      </p>
    </div>
  );
}
