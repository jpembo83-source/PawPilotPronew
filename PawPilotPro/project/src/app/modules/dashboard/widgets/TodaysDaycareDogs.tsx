...
import { Dog, MagnifyingGlass, ArrowClockwise, Warning, FileDashed, Prohibit, Pulse, Clock, CaretRight } from '@phosphor-icons/react';
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex-1 overflow-y-auto -mx-4 px-4">
            ...
            {isLoading && sortedBookings.length === 0 && (
              <div className="text-center py-12">
                <ArrowClockwise className="h-8 w-8 text-slate-300 mx-auto mb-2 animate-spin" />
    ...
    >
      <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 mb-1">
            ...
            {hasAnyAlert && (
              <div className="flex items-center gap-1">
                {alerts.hold && <Prohibit key="hold" className="h-3.5 w-3.5 text-red-500" title="Hold" />}
                {(alerts.vaccination || alerts.waiver) && <FileDashed key="docs" className="h-3.5 w-3.5 text-amber-500" title="Document Alert" />}
                {alerts.behaviour && <Warning key="behaviour" className="h-3.5 w-3.5 text-orange-500" title="Behaviour Flag" />}
                {alerts.medical && <Pulse key="medical" className="h-3.5 w-3.5 text-red-500" title="Medical Flag" />}
              </div>
            )}
          </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <CaretRight className="h-4 w-4 text-slate-300 group-hover:text-primary transition-colors" />
        </div>
      </div>
    </button>
  );
