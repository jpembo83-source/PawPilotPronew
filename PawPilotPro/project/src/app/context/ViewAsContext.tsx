...
export function ViewAsProvider({ children }: { children: React.ReactNode }) {
  // Safely get auth context - handle case where provider isn't ready yet
  ...
  useEffect(() => {
    ...
    const loadActiveSession = async () => {
      ...
      try {
        ...
        if (mounted) {
          console.debug('View As: Could not load active session (this is normal on first load)', err);
          store.setSession(null);
          store.setTargetUser(null);
          store.setError(null);
        }
      }
    };
  ...
  }, [user?.id, authLoading, store]); // Include store in dependencies
