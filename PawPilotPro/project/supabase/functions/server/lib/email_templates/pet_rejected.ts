export function petRejectedEmail(args: {
  ownerName: string;
  tenantName: string;
  petName: string;
  reason: string;
  ...
  const subject = `${args.tenantName} — we couldn't verify ${args.petName}`;
  const html = `
    <div style="font-family:-apple-system,BlinkMacSystemFont,sans-serif;max-width:480px;margin:0 auto;padding:24px;">
      ...
      <p style="color:#444;line-height:1.5;">We couldn't verify <strong>${args.petName}</strong>'s details, so bookings aren't open for them yet.</p>
      ...
      <p style="color:#444;">Get in touch with the team, or update ${args.petName}'s details in your portal, and we'll take another look.</p>
  ...
  return { subject, html, text: `We couldn't verify ${args.petName}: ${args.reason}. ${args.portalUrl}` };
}
