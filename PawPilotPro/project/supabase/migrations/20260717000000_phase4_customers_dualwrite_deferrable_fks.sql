-- ============================================================================
-- Phase 4 / Customers — STAGE 2 fix: make the dual-write applier tolerant of
-- intra-batch foreign-key ordering.
--
-- Root cause (2026-07-12 bulk /import drift, ~1,700 rows in KV, 0 mirrored):
--   public.phase4_customers_apply(ops) applies ops in array order in ONE
--   transaction. A bulk import stages a household together with the brand-new
--   primary contact it created in the same import, and the household blob
--   carries primary_contact_id pointing at that contact. The household op is
--   emitted before the contact op, so the household INSERT referenced a
--   contact row that did not yet exist *in that transaction*. households_
--   primary_contact_fk is a plain immediate, validated FK, so the INSERT
--   raised at once and the whole batch rolled back — non-fatally (the caller
--   logs phase4.customers.dualwrite.failed and lets the request succeed), so
--   KV kept the rows and Postgres got none. This is a cyclic dependency
--   (households.primary_contact_id → contacts, contacts.household_id →
--   households) that no single-pass op order can satisfy — exactly why the
--   stage-1 backfill inserts households with primary_contact_id NULL, loads
--   contacts, then restores it.
--
-- Fix: make the customer-table FKs DEFERRABLE INITIALLY IMMEDIATE and have the
--   applier SET CONSTRAINTS ALL DEFERRED, so every FK is checked once at
--   COMMIT rather than per-statement. Any op order within the batch is then
--   valid as long as the batch is internally consistent at commit; a genuinely
--   dangling reference still fails the whole batch at commit (integrity is
--   preserved, never weakened). INITIALLY IMMEDIATE keeps default behaviour
--   identical for every other code path — only the applier's own transaction
--   defers. No route change and no chunking: chunking would split a household
--   from its primary contact across transactions and reintroduce the failure.
-- ============================================================================

-- 1. Make the FKs on the customer graph deferrable (default stays immediate).
alter table public.contacts
  alter constraint contacts_household_id_fkey deferrable initially immediate;
alter table public.pets
  alter constraint pets_household_id_fkey deferrable initially immediate;
alter table public.customer_documents
  alter constraint customer_documents_household_id_fkey deferrable initially immediate;
alter table public.customer_documents
  alter constraint customer_documents_pet_id_fkey deferrable initially immediate;
alter table public.household_notes
  alter constraint household_notes_household_id_fkey deferrable initially immediate;
alter table public.household_flags
  alter constraint household_flags_household_id_fkey deferrable initially immediate;
alter table public.household_flags
  alter constraint household_flags_pet_id_fkey deferrable initially immediate;
alter table public.note_pets
  alter constraint note_pets_note_id_fkey deferrable initially immediate;
alter table public.note_pets
  alter constraint note_pets_pet_id_fkey deferrable initially immediate;
alter table public.households
  alter constraint households_primary_contact_fk deferrable initially immediate;

-- 2. Re-declare the applier with FK checks deferred to commit. Body is byte-for
--    -byte identical to 20260711220000 apart from the single SET CONSTRAINTS
--    statement at the top of the transaction.
...
declare
  ...
  fam  text;
begin
  -- Defer every deferrable FK (only the customer-graph FKs above are
  -- deferrable) so intra-batch op order cannot raise mid-transaction; the
  -- cyclic household ↔ contact reference is validated once at COMMIT.
