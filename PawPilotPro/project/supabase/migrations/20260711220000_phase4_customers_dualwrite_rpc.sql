-- ============================================================================
-- Phase 4 / Customers — STAGE 2: transactional dual-write applier.
--
-- public.phase4_customers_apply(ops jsonb) mirrors a batch of KV mutations
-- onto the stage-0 customer tables in ONE transaction (an RPC call is a
-- single transaction), so a multi-key flow (primary-contact flip, cascade
-- delete) can never partially apply on the Postgres side.
--
-- Each op mirrors a KV verb exactly:
--   {"op":"set","key":"customer:…","value":{…}}  → typed full-row upsert
--   {"op":"del","key":"customer:…"}              → delete WHERE legacy_kv_key = key
--
-- Semantics deliberately mirror kv_store set/del, not "intent":
--   * The key is classified with the SAME segment rules as the stage-1
--     backfill (20260702093000); an unclassifiable key raises (loud, caught
--     and logged non-fatally by the caller — see lib/customers_dualwrite.ts).
--   * Field extraction per family matches the backfill's INSERT lists, so
--     stage-1 rows and stage-2 rows are shaped identically.
--   * legacy_kv_key is stamped on every upsert; deletes match on it. This
--     keeps the stage-1 parity equation (KV keys = rows + quarantined) true
--     for dual-written rows, and makes deletes byte-faithful to kv.del
--     (an activity stored under the 5-segment key is untouched by a del of
--     the 4-segment key, exactly as in KV).
--   * note_pets carries no legacy_kv_key; both ends come from the key.
--   * Casts are strict (::timestamptz, ::numeric, ::boolean). A malformed
--     value fails the whole batch — visible in logs and in the drift check,
--     never silently coerced. KV remains authoritative throughout stage 2.
--
-- Callers: the edge function only (service_role). EXECUTE is revoked from
-- anon/authenticated below — this is not a client-facing RPC.
-- ============================================================================
