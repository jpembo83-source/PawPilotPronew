// Axe audit over the four hot-path screens, desktop + mobile viewports.
// Same fake-session + fixture harness as shots.mjs.
// Usage: node a11y-audit.mjs <outFile.json> [baseUrl]
import { chromium } from '@playwright/test';
import AxeBuilder from '@axe-core/playwright';
import { writeFileSync } from 'node:fs';

const outFile = process.argv[2] || 'a11y-report.json';
...
const jwt = `${b64({ alg: 'none', typ: 'JWT' })}.${b64({
  ...
  exp: now + 6 * 3600, email: userObj.email,
  app_metadata: userObj.app_metadata, user_metadata: userObj.user_metadata,
})}.fake`;
const session = {
  access_token: jwt, token_type: 'bearer', expires_in: 6 * 3600,
  expires_at: now + 6 * 3600, refresh_token: 'fake-refresh', user: userObj,
};
...
const onSite = [3, 4].map((n) =>
  mkBooking(n, {
    ...
    medical_notes: n === 3 ? 'Grass allergy.' : undefined,
  })
);
...
const pet = {
  ...
  medical_notes: 'Daily anti-seizure medication at 12:00.',
  allergies: 'Chicken — strict.',
...
async function route(routeObj) {
  ...
  if (url.includes('/capacity/daily')) return json({ locations: [], date: today });
  if (url.includes('/capacity/weekly')) return json({ days: [], location_id: 'loc-1' });
  if (url.includes('/search-customers')) return json([]);
  if (url.includes('/validate-checkin')) return json({ can_check_in: true, blockers: [], warnings: [] });
...
const screens = [
  ...
  { path: '/daycare/check-out', name: 'check-out' },
...
for (const vp of viewports) {
  ...
  for (const s of screens) {
    ...
    report.results.push({ screen: s.name, viewport: vp.label, violations, passes: axe.passes.length });
    const serious = violations.filter(v => v.impact === 'critical' || v.impact === 'serious');
    console.log(`${s.name} @ ${vp.label}: ${violations.length} violations (${serious.length} critical/serious)`);
    for (const v of serious) console.log(`   [${v.impact}] ${v.id} ×${v.nodeCount}`);
  }
  await ctx.close();
}
await browser.close();
