...
async function route(routeObj) {
  ...
  if (url.includes('/search-customers')) return json([
    { type: 'pet', household_id: 'hh-1', household_name: 'The Bennett Household', matched_pet_id: 'pet-1', contacts: [],
      pets: [
        { id: 'pet-1', name: 'Biscuit', breed: 'Cocker Spaniel', vaccination_status: 'valid', medical_notes: 'Daily anti-seizure medication at 12:00.' },
        { id: 'pet-9', name: 'Bella', breed: 'Labrador', vaccination_status: 'valid' },
      ] },
    { type: 'household', household_id: 'hh-2', household_name: 'The Bishop Household', contacts: [],
      pets: [ { id: 'pet-10', name: 'Bruno', breed: 'Boxer', vaccination_status: 'valid', behaviour_notes: 'Pulls hard on lead.' } ] },
  ]);
...
for (const vp of viewports) {
  ...
  await page.goto(`${baseUrl}/daycare/attendance`, { waitUntil: 'load', timeout: 60000 });
  await page.waitForTimeout(2000);
  if (vp.label === 'mobile') {
    await page.getByLabel('Search pets and households').click();
  } else {
  ...
  await page.screenshot({ path: `${outDir}/search-${vp.label}.png` });
  console.log(`captured search-${vp.label}.png`);
